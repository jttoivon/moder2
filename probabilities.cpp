...
max_matrix_logodds_score(const dmatrix& m)
{
  ...
  double max_sum=0;
  for (int i=0; i<k; ++i) {
    // double col_max=-10000;
    // for (int j=0; j < 4; ++j)
    //   col_max = std::max(col_max, m(j,i));
    max_sum += max_element(m.column(i));
  }
  return max_sum;
}
...
count_background(const std::vector<std::string>& sequences)
{
  ...
  */
  return boost::make_tuple(background_frequencies, background_frequency_matrix);
}
